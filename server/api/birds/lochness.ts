...
export default cachedEventHandler(
  async (event): Promise<BirdsResponse> => {
    ...
    const back = Math.min(Math.max(Number(query.back) || 1, 1), 30) // clamp 1–30

    const target = new Date()
    target.setDate(target.getDate() - (back - 1))
      ...
      Effect.tryPromise({
        try: () =>
          $fetch<EBirdObservation[]>(`https://api.ebird.org/v2/data/obs/${locationId}/recent`, {
            ...
            query: { back, includeProvisional: true },
          }),
  ...
  },
  {
    ...
    getKey: (event) => {
      const query = getQuery(event)
      const back = Math.min(Math.max(Number(query.back) || 1, 1), 30)
